...
template<typename T>
inline constexpr bool tpcc_is_fixed_byte_array_v = false;

template<std::size_t length, auto low>
inline constexpr bool
    tpcc_is_fixed_byte_array_v<
	t_fixedarray<t_byte, length, low>> = true;
...
inline auto& tpcc_byte_array_storage_view(
    Scalar&& value) {
	using source_type =
	    std::remove_reference_t<Scalar>;
	using bare_source_type =
	    std::remove_const_t<source_type>;
	using view_type = std::conditional_t<
	    std::is_lvalue_reference_v<Scalar&&> &&
		!std::is_const_v<source_type>,
	    Array,
	    const Array>;
	static_assert(
	    tpcc_is_fixed_byte_array_v<Array>,
	    "Pascal object-representation view must be a fixed Byte array");
	static_assert(
	    std::is_same_v<t_byte, unsigned char>,
	    "Pascal Byte object-representation views require unsigned char");
	static_assert(
	    std::is_trivially_copyable_v<bare_source_type>,
	    "Pascal Byte-array view source must be trivially copyable");
	static_assert(
	    std::is_trivially_copyable_v<Array>,
	    "Pascal fixed Byte array must be trivially copyable");
	static_assert(
	    sizeof(Array) == sizeof(bare_source_type),
