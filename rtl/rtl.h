...
struct t_tobject {
	struct m_tobject: public m_iobject { // = t_tclass maybe
		virtual ~m_tobject() = default;
		virtual m_iobject* classtype() {
			static m_iobject meta{};
			return &meta;
		}
	...
	//std::unique_ptr<m_iobject> meta = std::make_unique<m_tobject>();
...
struct m_tfoo: public m_tobject {
    ...
    virtual m_tobject* p_classparent() { // FIXME: class method
        return m_tobject::p_classtype();
    }
