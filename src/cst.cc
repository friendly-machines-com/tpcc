...
RangeCheckedCast::RangeCheckedCast(Node* value, Type* target, RangeCheckDisposition disposition) : Cast(value, target), disposition(disposition) {
}
...
ConstEvalResult RangeCheckedCast::const_eval(ConstEvalContext& ctx) const {
	if (disposition == RangeCheckDisposition::AlwaysFail) {
		// The selected target is known, but an exact origin has no runtime
		// carrier to inspect. Preserve the selected failure as the operation
		// itself instead of inventing a source representation.
		if (auto real = dynamic_cast<Real*>(a); real && real->is_origin() && is_real_semantic_type(ty)) {
			// Keep the established real-materialization diagnostic even
			// though this node records the failure without a materialized
			// source value.
			return ConstEvalResult::error("real constant out of range for target type");
		}
		return ConstEvalResult::error("constant out of range for target type");
	}
