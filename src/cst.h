...
class Construct : public Node {
	...
	Method* initializer;
	std::vector<Node*> args;
	Construct(Node* class_reference, Method* initializer, std::vector<Node*> args, ClassType* result_type);
	const char* diagnostic_kind() const override;
	void collect_diagnostic_edges(ErrorLetContext* ctx) const override;
	void print_diagnostic_definition(ErrorLetContext* ctx, std::ostringstream& out, unsigned indent) const override;
};
...
class NewValue : public Node {
	...
	Method* initializer;
	std::vector<Node*> args;
	NewValue(Type* pointer_type, Type* allocated_type, Method* initializer, std::vector<Node*> args);
	const char* diagnostic_kind() const override;
	void collect_diagnostic_edges(ErrorLetContext* ctx) const override;
	void print_diagnostic_definition(ErrorLetContext* ctx, std::ostringstream& out, unsigned indent) const override;
};
...
class WriteCall : public Node {
	...
	WriteCall(bool newline, Node* file, const BuiltinDesc* lowering_builtin_desc, std::vector<FormattedValue> items);
	const char* diagnostic_kind() const override;
	void collect_diagnostic_edges(ErrorLetContext* ctx) const override;
	void print_diagnostic_definition(ErrorLetContext* ctx, std::ostringstream& out, unsigned indent) const override;
};
...
class StrCall : public Node {
	...
	StrCall(FormattedValue formatted, Node* destination);
	const char* diagnostic_kind() const override;
	void collect_diagnostic_edges(ErrorLetContext* ctx) const override;
	void print_diagnostic_definition(ErrorLetContext* ctx, std::ostringstream& out, unsigned indent) const override;
};
...
class Mutation : public Node {
	...
	Mutation(Node* source_target, std::vector<Binding> bindings, Node* target, StorageSlot* current, Assign* assignment);
	const char* diagnostic_kind() const override;
	void collect_diagnostic_edges(ErrorLetContext* ctx) const override;
	void print_diagnostic_definition(ErrorLetContext* ctx, std::ostringstream& out, unsigned indent) const override;
};
...
class Raise : public Node {
	...
	Node* address;
	Node* frame;
	Raise(Node* object, Node* address = nullptr, Node* frame = nullptr);
	const char* diagnostic_kind() const override;
	void collect_diagnostic_edges(ErrorLetContext* ctx) const override;
	void print_diagnostic_definition(ErrorLetContext* ctx, std::ostringstream& out, unsigned indent) const override;
};
...
class ConstantDecl : public Node {
	...
	Node* initializer;
	Type* owner_type;
	ConstantDecl(std::string cxx_name, Type* ty, Node* initializer, Type* owner_type);
	const char* diagnostic_kind() const override;
	ConstEvalResult const_eval(ConstEvalContext& ctx) const override;
	void collect_diagnostic_edges(ErrorLetContext* ctx) const override;
	void print_diagnostic_definition(ErrorLetContext* ctx, std::ostringstream& out, unsigned indent) const override;
...
class Integer : public Node {
	...
	Integer(uint64_t value, Type* ty, bool negative = false, bool based_literal = false);
	const char* diagnostic_kind() const override;
	ConstEvalResult const_eval(ConstEvalContext& ctx) const override;
	void print_diagnostic_definition(ErrorLetContext* ctx, std::ostringstream& out, unsigned indent) const override;
};
...
class Real : public Node {
      public:
	// Present exactly when ty is untyped_real_type(). Typed real constants
	// store their already-rounded concrete value in `value`.
	std::optional<DecimalOrigin> origin;
	long double value = 0.0L;
	Real(long double value, Type* ty);
	explicit Real(DecimalOrigin origin);
...
class BracketLiteral : public Node {
	...
	BracketLiteral(std::vector<Item> items, Type* default_set_item_type, FixedArrayType* default_array_type);
	const char* diagnostic_kind() const override;
	ConstEvalResult const_eval(ConstEvalContext& ctx) const override;
	void collect_diagnostic_edges(ErrorLetContext* ctx) const override;
	void print_diagnostic_definition(ErrorLetContext* ctx, std::ostringstream& out, unsigned indent) const override;
...
class ArrayLiteral : public Node {
      public:
	std::vector<Node*> elements;
	ArrayLiteral(std::vector<Node*> elements, Type* ty);
	const char* diagnostic_kind() const override;
	ConstEvalResult const_eval(ConstEvalContext& ctx) const override;
	void collect_diagnostic_edges(ErrorLetContext* ctx) const override;
	void print_diagnostic_definition(ErrorLetContext* ctx, std::ostringstream& out, unsigned indent) const override;
...
class ValueBound : public UnaryOperation {
      public:
	TypeBoundKind kind;
	ValueBound(TypeBoundKind kind, Node* value, Type* result_type);
	const char* diagnostic_kind() const override;
	ConstEvalResult const_eval(ConstEvalContext& ctx) const override;
};
...
class BuiltinEnumeratorCurrent : public Node {
      public:
	explicit BuiltinEnumeratorCurrent(Type* element_type);
	const char* diagnostic_kind() const override;
};
...
class Callable : public Node {
	...
	bool has_body = false;
	Frame* body_frame;
	Callable(std::string cxx_name, std::string pas_name, RoutineType* ty, bool has_overload_directive);
...
class Procedure : public Callable {
      public:
	Procedure(std::string cxx_name, std::string pas_name, RoutineType* ty, bool has_overload_directive);
	const char* diagnostic_kind() const override;
};
...
class Method : public Callable {
	...
	Method(std::string cxx_name, std::string pas_name, RoutineType* ty, bool has_overload_directive, Type* owner_class, VirtualKind virtual_kind);
	const char* diagnostic_kind() const override;
	void collect_diagnostic_edges(ErrorLetContext* ctx) const override;
	void print_diagnostic_definition(ErrorLetContext* ctx, std::ostringstream& out, unsigned indent) const override;
};
