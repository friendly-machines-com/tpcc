...
bool eval_directive_expr(const std::string& expr, const std::map<std::string, std::string, CILess>& defines) {
	...
	auto lookup_ident = [&](const std::string& name) -> int64_t {
		...
		if (s.empty()) {
			throw DirectiveExprError{"identifier '" + name + "' has no value (defined without :=) but is used numerically in {$if ...}"};
		}
		int64_t v;
		auto [ptr, ec] = std::from_chars(s.data(), s.data() + s.size(), v);
		if (ec != std::errc() || ptr != s.data() + s.size()) {
			throw DirectiveExprError{"value '" + s + "' of '" + name + "' is not an integer in {$if ...}"};
		}
		return v;
	};
	...
	skip_ws();
	if (p != expr.size()) {
		throw DirectiveExprError{"unrecognised trailing form in {$if ...}: '" + expr.substr(p) + "'"};
	}
