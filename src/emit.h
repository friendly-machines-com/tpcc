...
class Emitter {
	...
	void emit_return_transfer_handler(unsigned try_depth, RoutineType* routine);
	...
	void emit_unit_interface_prologue(std::string unit_namespace, std::vector<std::string> used_unit_h_files);
	...
	// #includes.
	void emit_unit_implementation_prologue(std::string unit_namespace, std::string this_unit_h_file, std::vector<std::string> impl_used_unit_h_files);
	...
	void emit_class_forward_declaration(std::string cxx_name);
	...
	void emit_main_epilogue(bool has_program_class_destructors);
	...
	void emit_exception_handler_prologue(Type* exception_type, std::string variable_cxx_name, bool first);
	void emit_exception_handler_epilogue();
	void emit_exception_default_prologue();
	void emit_exception_default_epilogue();
	void emit_try_except_epilogue(bool typed_handlers, bool has_default);
	void emit_try_finally_prologue();
	void emit_try_finally_epilogue();
	void emit_try_control_epilogue(unsigned try_depth, RoutineType* routine, bool inside_loop);
	...
	void emit_for_in_cleanup_control_epilogue(unsigned try_depth, RoutineType* routine);
	...
	void emit_for_prologue(Node* control, Node* initial, Node* final, bool descending, bool overflow_checks);
	void emit_for_epilogue();
	void emit_for_in_sequence_prologue(Node* collection, Node* current_assignment);
	void emit_for_in_set_prologue(Node* collection, Node* lower, Node* upper, Node* current_assignment);
	void emit_for_in_ordinal_prologue(Type* ordinal_type, Node* lower, Node* upper, Node* current_assignment);
	void emit_for_in_custom_setup(Node* get_enumerator, bool nullable);
	void emit_for_in_custom_loop_prologue(Node* move_next, Node* current_assignment);
	void emit_for_in_loop_epilogue();
	void emit_for_in_custom_epilogue(bool nullable);
	void emit_for_in_epilogue();
	void emit_loop_control(bool is_break, unsigned try_depth = 0, unsigned target_try_depth = 0);
	...
	void emit_routine_signature(RoutineType* ty, std::string cxx_text, Position pos, std::string owner_qualifier, bool cxx_destructor, Type* conversion_operator_target);
	...
	void emit_aggregate_member_fields(const std::vector<AggregateField>& fields);
	...
	void emit_type_dependencies(Type* ty, bool inspect_definition = false);
	void emit_subrange_definition(SubrangeType* subrange);
	...
	void emit_formal_parameters(RoutineType* ty, bool with_names, Type* conversion_operator_target);
	...
	void emit_call_arguments(RoutineType* ty, const std::vector<Node*>& args);
	void emit_formatted_value(const FormattedValue& formatted);
	...
	void emit_static_member_declaration(StorageSlot* slot);
	...
	void emit_record_variant_decl(VariantPart* variant, unsigned indent, unsigned depth);
