...
static int test_text_eof_error() {
	std::FILE* handle = std::fopen("/dev/null", "wb");
	if (!handle) {
		return 1;
	}
	::u_system::t_text file{new ::u_system::text_file_state{
	    .name = {},
	    .handle = handle,
	    .mode = ::u_system::text_file_mode::Input,
	...
	::u_system::m_unchecked_write(file, ::u_system::tpcc_make_formatted_value(static_cast<::u_system::t_integer>(1)));
...
static int test_text_readln_error() {
	...
	::u_system::m_unchecked_readln(file, line);
	if (!consume_pascal_error()) {
		return 2;
	}
	if (std::ferror(handle)) {
...
static int test_blockread_error() {
	...
	file.state = new ::u_system::binary_file_state{
	    .name = {},
	    .handle = handle,
	    .record_size = 1,
	    .readable = true,
	...
	::u_system::m_unchecked_blockread(file, ::u_system::tpcc_make_storage_ref(value), static_cast<::u_system::t_longint>(1), transferred);
	if (!consume_pascal_error()) {
		return 2;
	}
	if (std::ferror(handle)) {
	...
	::u_system::m_unchecked_blockwrite(file, ::u_system::tpcc_make_const_storage_ref(value), static_cast<::u_system::t_longint>(1), transferred);
	if (::u_system::p_ioresult() != 0 || transferred != 1) {
		return 4;
	}
	return 0;
}
...
static int test_blockwrite_error() {
	std::FILE* handle = std::fopen("/dev/null", "rb");
	if (!handle) {
		return 1;
	}
	::u_system::t_file file;
...
static int test_write_error() {
	std::FILE* handle = std::fopen("/dev/full", "wb");
	if (!handle) {
		return 1;
	}
	if (std::setvbuf(handle, nullptr, _IONBF, 0) != 0) {
		std::fclose(handle);
		return 2;
	}
	::u_system::t_text file{new ::u_system::text_file_state{
...
static int test_flush_error() {
	...
	char buffer[BUFSIZ];
	if (std::setvbuf(handle, buffer, _IOFBF, sizeof(buffer)) != 0) {
		std::fclose(handle);
		return 2;
	}
...
int main() {
	const int eof_error = test_text_eof_error();
	if (eof_error != 0) {
		return 10 + eof_error;
	}
	if (::u_system::p_ioresult() != 0) {
	...
	const int readln_error = test_text_readln_error();
	if (readln_error != 0) {
	...
	const int blockread_error = test_blockread_error();
	...
	const int blockwrite_error = test_blockwrite_error();
	...
	const int write_error = test_write_error();
	if (write_error != 0) {
	...
	const int flush_error = test_flush_error();
	if (flush_error != 0) {
