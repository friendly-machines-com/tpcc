...
static bool matches(
    const system_time& value,
    const ::timespec& snapshot) {
	std::tm local{};
	if (!::localtime_r(&snapshot.tv_sec, &local))
		return false;
	return value.p_year == local.tm_year + 1900 &&
	       value.p_month == local.tm_mon + 1 &&
	       value.p_dayofweek == local.tm_wday &&
	       value.p_day == local.tm_mday &&
	       value.p_hour == local.tm_hour &&
	       value.p_minute == local.tm_min &&
	       value.p_second == local.tm_sec;
}
