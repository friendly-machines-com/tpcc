...
int main() {
	::u_system::t_pointer zeroed = ::u_system::p_allocmem(16);
	if (!zeroed) {
		return EXIT_FAILURE;
	}
	auto* bytes = static_cast<unsigned char*>(zeroed);
	...
	::u_system::t_char* returned = ::u_system::p_reallocmem(characters, 16);
	...
	if (returned != characters || characters[0] != ::u_system::t_char{'a'}) {
		return EXIT_FAILURE;
	}
	...
	if (::u_system::p_reallocmem(characters, 0) != nullptr || characters != nullptr) {
		return EXIT_FAILURE;
	}
	...
	::u_system::t_pointer raw_returned = ::u_system::p_reallocmem(raw, 8);
