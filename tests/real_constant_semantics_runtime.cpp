...
int main() {
	...
	if (p_exactoriginsize != sizeof(::u_system::t_single) || p_roundedoriginsize != sizeof(::u_system::t_extended))
		return 13;
	if (p_roundedintegerrank != 2 ||
	    p_wideintegerrank != 3 ||
	    p_wideintegermixedrank != 3)
		return 14;
	if (p_tinysingle != 0.0f || !__builtin_isfinite(p_tinysingle) ||
	    p_tinydouble != 0.0 || !__builtin_isfinite(p_tinydouble) ||
	    p_tinyextended != 0.0L || !__builtin_isfinite(p_tinyextended))
		return 15;
	return 0;
}
