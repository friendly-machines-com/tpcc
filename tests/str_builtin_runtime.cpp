...
static bool equals(const ::u_system::t_ansistring& value, const char* expected) {
	return value.m_string() == expected;
}

template <typename T> static std::string render(const ::u_system::tpcc_formatted_value<T>& value) {
	const auto rendered = ::u_system::tpcc_render_formatted_value(value);
	return std::string(rendered.left_padding, ' ') + rendered.value;
}
...
int main() {
	...
	// sinks differ.
	auto integer = ::u_system::tpcc_make_formatted_value(static_cast<::u_system::t_integer>(42), static_cast<::u_system::t_sizeint>(5));
	::u_system::p_str(integer, text);
	if (!equals(text, render(integer).c_str())) {
		return EXIT_FAILURE;
	}

	auto extended = ::u_system::tpcc_make_formatted_value(static_cast<::u_system::t_extended>(1.5L));
	::u_system::p_str(extended, text);
	if (!equals(text, render(extended).c_str())) {
		return EXIT_FAILURE;
	}

	auto precise = ::u_system::tpcc_make_formatted_value(static_cast<::u_system::t_extended>(1.5L), static_cast<::u_system::t_sizeint>(0), static_cast<::u_system::t_sizeint>(3));
	::u_system::p_str(precise, dynamic_text);
	if (!equals(dynamic_text, "1.500")) {
		return EXIT_FAILURE;
	}

	auto wide = ::u_system::tpcc_make_formatted_value(static_cast<::u_system::t_integer>(7), static_cast<::u_system::t_sizeint>(300));
	::u_system::p_str(wide, dynamic_text);
	if (dynamic_text.m_length() != 300 || dynamic_text.index(1).value != ' ' || dynamic_text.index(299).value != ' ' || dynamic_text.index(300).value != '7') {
		return EXIT_FAILURE;
	}
