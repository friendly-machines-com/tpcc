...
static bool parse(const char* text, std::size_t length, T expected) {
	::u_system::t_shortstring<255> source =
	    ::u_system::tpcc_shortstring_from_c(text, length);
	T value{};
...
int main() {
	...
	::u_system::t_shortstring<255> source =
	    ::u_system::tpcc_shortstring_from_c("128", 3);
	...
	::u_system::p_val(
	    source,
	    integer,
	    ::u_system::tpcc_make_storage_ref(byte_code));
