...
int main() {
	...
	std::FILE* old_output =
	    ::u_system::m_stdout_text_state.handle;
	std::FILE* old_error =
	    ::u_system::m_stderr_text_state.handle;
	::u_system::m_stdout_text_state.handle =
	    standard_output;
	::u_system::m_stderr_text_state.handle =
	    standard_error;
	...
	if (result != 0)
		return 2;
	if (read_output(standard_output) !=
	    "A12 Z!\n\nTRUE\n  12\nexplicit stdout")
		return 3;
	if (read_output(standard_error) !=
	    "explicit stderr\n")
		return 4;
	if (read_output(file_output) !=
	    "file=-7:2.50\n")
		return 5;

	::u_system::m_release_text_file_state(
	    p_destination.state);
	...
	std::FILE* failed_output =
	    std::fopen("/dev/full", "wb");
	if (!failed_output)
		return 6;
	std::setvbuf(
	    failed_output, nullptr, _IONBF, 0);
